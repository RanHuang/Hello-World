...
if __name__ == '__main__':
    ...
    for row in rows_list:
        ...
        overtime = row[COLUMN_OVER_TIME].value
        if overtime is None:
            # 不存在加班打卡记录
            row[COLUMN_OVER_TIME_STATISTIC].value = 0
        else:
            ...
            if len(overtime_list) != 2:
                row[COLUMN_COMMENT].value = "打卡次数异常(周内必须2次加班打卡记录)"
                row[COLUMN_OVER_TIME_STATISTIC].value = -1
                continue
            ...
            row[COLUMN_OVER_TIME_STATISTIC].value = overtime_statistic
            row[COLUMN_OVER_TIME].value = TIME_SEPARATOR.join([overtime_start, overtime_last])

    # 计算周末加班时间
    for row in rows_list:
        if not row[COLUMN_IS_WEEKEND].value:
            # 跳过周内加班数据
            continue
        ...
        if overtime is None:
            ...
            if len(overtime_list) == 6:
                ...
                row[COLUMN_COMMENT].value = "周末加班打卡次数异常(正常次数为2,4,6)"
                overtime_statistic = -1
            row[COLUMN_OVER_TIME_STATISTIC].value = overtime_statistic
            row[COLUMN_OVER_TIME].value = TIME_SEPARATOR.join(overtime_list)
